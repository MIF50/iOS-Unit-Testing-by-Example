...
class MockPasswordChanger: PasswordChanging {
    ...
    func verifyChangeNeverCalled(file: StaticString = #filePath,line: UInt = #line) {
        changeWasNeverCalled(file: file, line: line)
    }
    
    func changeCallSuccess(file: StaticString = #filePath, line: UInt = #line) {
        guard changeWasCalledOnce(file: file, line: line) else { return }
        
        changeArgsOnSuccess.last!()
    }
    
    func changeCallFailure(message: String,file: StaticString = #filePath, line: UInt = #line) {
        guard changeWasCalledOnce(file: file, line: line) else { return }
        
        changeArgsOnFailure.last!(message) }
    
    private func changeWasCalledOnce(file: StaticString = #filePath, line: UInt = #line) -> Bool {
    ...
    private func changeWasNeverCalled(file: StaticString = #filePath, line: UInt = #line) {
